...
Ask the user, with these answers:

- **"Describe it:"** — free text
- **"Use an existing agent file"** — then ask: *"Where can I find the existing agent?"*

If the user already passed a description or a path as an argument, skip the question and confirm what you understood.

For an imported file: read it now. Keep its role — you will rewrite everything around it in Step 6.
...
Present your findings, with these answers:

- **the spots you found**, each with a one-line reason, one marked as recommended
- **"Extend `<existing agent>` instead"** — if Step 2 showed the job is already covered
- **"Somewhere else:"** — free text
...
If the answer is "the whole team", ask next: *"You need a copy of the plugin repo. Where is it?"*

- **"Here:"** — a path. Verify it holds `.claude-plugin/plugin.json` and `agents/`, and that the working tree is clean.
...
Then ask: **go** / **adjust** / **cancel**.

Write nothing before "go". On "adjust", take the correction and show the preview again.
